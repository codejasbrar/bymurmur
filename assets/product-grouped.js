...
// console.log(cbbSelects);
    ...
    var selectors = document.body.querySelectorAll('li.cbb-also-bought-product');
        $(selectors).each(function(index,items) {
          var h3 = $(items).find('h3 a').text().split(',');
          h3 = h3[0].replace(" ","-");
          $(items).addClass(h3);
        })
        ...
        container.before(selectVariantLabel)
                  console.log("selectVariantLabel",selectVariantLabel.textContent);
        ...
        options.forEach((option) => {
          ...
          var optionNaame = option.textContent;
          var before_name = optionNaame.substring(0, optionNaame.indexOf('/'));
          if(before_name ==""){
            var before_name = optionNaame;          
          }
          ...
          button.classList.add(color_v)
          button.classList.add(before_name)
          // if(option.value < 1){
          ...
          button.style.background = `${option.textContent.split('/').pop().toLowerCase().trim().replace(/ /, '-')}`
      ...
      addToCartForm.addEventListener('submit', (evt) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          let qty = parseInt($(itemData).find('[name="quantity"]').val())
//           let qty = parseInt(itemData.querySelector('[name="quantity"]').value)
      ...
      buttons.forEach((button) => {
        ...
        button.addEventListener('click', function () {
          ...
          $(select).find('[data-variant-id="'+variantId+'"]').attr('selected','selected')

          var seected = $(select).find(':selected').attr('data-variant-id');
          
        /*  console.log("seected==>",seected)
